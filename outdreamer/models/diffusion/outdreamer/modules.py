...
class Attention(Attention_):
    def __init__(self, downsampler, attention_mode, use_rope, interpolation_scale_thw, use_maskNet = False, mask_block = None, **kwags):
        processor = AttnProcessor2_0(attention_mode=attention_mode, use_rope=use_rope, interpolation_scale_thw=interpolation_scale_thw, use_maskNet = use_maskNet, mask_block = mask_block)
...
class AttnProcessor2_0:
    ...
    def __init__(self, attention_mode='xformers', use_rope=False, interpolation_scale_thw=(1, 1, 1), use_maskNet = False, mask_block = None):
        ...
        if npu_config is not None and npu_config.on_npu:
            if get_sequence_parallel_state():
                ...
                if self.use_mask == True:
                    down_mask = F.interpolate(self.mask, size=(8, 60, 80), mode='trilinear', align_corners=False).flatten()
                    W = self.mask_block.cuda()(down_mask).to(key.dtype)
                    W = 1.0 + (W - 0.5) * 2.0 * self.scale
            ...
            if get_sequence_parallel_state():
                ...
                if self.use_mask == True:
                    down_mask = F.interpolate(self.mask, size=(8, 60, 80), mode='trilinear', align_corners=False).flatten()
                    W = self.mask_block.cuda()(down_mask.to(key.dtype)).to(key.dtype)
                    W = 1.0 + (W - 0.5) * 2.0 * self.scale
                    W = torch.unsqueeze(W, 1)
                    key = key * W
...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        input_length = 38400 # (8 * 60 * 80 = 38400) support for 29x640x460 video
        output_length = 9600 # (38400 / 4 = 9600)
        self.mask_block = nn.Sequential(
            nn.Linear(input_length, 200),
            nn.ReLU(),
            nn.Linear(200, 300),
            nn.ReLU(),
            nn.Linear(300, output_length),
            nn.Sigmoid() # [0, 1]
        )
