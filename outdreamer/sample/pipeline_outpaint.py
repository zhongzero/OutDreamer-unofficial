        ...
        **kwargs,
        ...
        for name, module in self.transformer.named_modules():
            if name.endswith("attn1"):
                module.processor.set_mask(input_mask[:1,:1])
                module.processor.set_scale(attention_scale)
        ...
        c, t, h, w = masked_video.shape[1], masked_video.shape[2], masked_video.shape[3], masked_video.shape[4]
        input_mask = F.interpolate(input_mask, size=(t, h, w), mode='trilinear', align_corners=False).to(latents.dtype)
        # input_mask = input_mask[:, :1]
        input_mask = input_mask[:, :1].repeat(1, 4, 1, 1, 1)
